...
- **コメント機能**: 各実行にメモを記録
- **コメント編集**: 履歴一覧から後からコメントを編集可能（このフォーク版で追加）
...
- **Seed一覧**: 入力特徴量を指定して該当seedを一覧表示（このフォーク版で追加）
...
- **ビジュアライザ連携**: 選択したテストケースをビジュアライザで直接表示
  ...
  - Scaleの値を変更してビジュアライザ部分の縮尺を調整可能（このフォーク版では初期倍率を小さめに調整）
...
- **Seed一覧表示**: 指定した入力値（例: N=5）に該当するseed一覧を表示

## このリポジトリについて

このリポジトリは **pahcer-studio のフォーク** です。以下はフォーク版で追加した主な改善点です。

- テスト履歴のコメントを後から編集できる
- ビジュアライザの初期倍率を小さめに調整
- 分析画面で入力特徴量に基づいた seed 一覧を表示
